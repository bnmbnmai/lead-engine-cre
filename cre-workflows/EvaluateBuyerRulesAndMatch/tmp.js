...
function evaluatePreferenceSet(lead, pref) {
  const result = {
...
function evaluateFieldFilters(parameters, rules) {
  const failedKeys = [];
  if (!rules || rules.length === 0)
    return { pass: true, failedKeys };
...
function evaluateSingleRule(leadValue, operator, filterValue) {
...
var fetchEvaluationData = (sendRequester, config) => {
  ...
  const results = [];
  for (let i2 = 0;i2 < data.preferenceSets.length; i2++) {
    results.push(evaluatePreferenceSet(data.lead, data.preferenceSets[i2]));
  }
  let matchedCount = 0;
  for (let i2 = 0;i2 < results.length; i2++) {
    if (results[i2].matched)
      matchedCount++;
  }
  return {
    leadId: data.lead.id,
...
var onCronTrigger = (runtime2) => {
  const confHTTPClient = new ClientCapability;
  const result = confHTTPClient.sendRequest(runtime2, fetchEvaluationData, consensusIdenticalAggregation())(runtime2.config).result();
  runtime2.log("--- EvaluateBuyerRulesAndMatch Results ---");
  runtime2.log(`Lead ID: ${result.leadId}`);
  runtime2.log(`Total preference sets evaluated: ${result.totalPreferenceSets}`);
  runtime2.log(`Matched sets: ${result.matchedSets}`);
  for (let i2 = 0;i2 < result.results.length; i2++) {
    const match = result.results[i2];
    if (match.matched) {
      runtime2.log(`  OK ${match.preferenceSetId} (buyer: ${match.buyerId}) $${match.suggestedBidAmount}`);
    } else {
      runtime2.log(`  SKIP ${match.preferenceSetId}: ${match.reason}`);
    }
  }
  runtime2.log(`Evaluated at: ${result.evaluatedAt}`);
  runtime2.log("---");
