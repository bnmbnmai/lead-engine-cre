...
function generateLanderHTML(config: z.infer<typeof LanderExportSchema>): string {
    ...
    const renderField = (f: z.infer<typeof FieldSchema>) => {
        ...
        const helpSpan = f.helpText ? `<span class="lf-help">${escapeHtml(f.helpText)}</span>` : '';

        let inputHtml: string;
        if (f.type === 'select' && f.options?.length) {
            inputHtml = `<select name="${escapeHtml(f.key)}"${req} class="lf-input"${autoFmtAttr}><option value="">Select ${lbl}</option>${f.options.map((o) => `<option value="${escapeHtml(o)}">${escapeHtml(o)}</option>`).join('')}</select>`;
        } else if (f.type === 'boolean') {
            inputHtml = `<label class="lf-toggle"><input type="checkbox" name="${escapeHtml(f.key)}"><span>${lbl}</span></label>`;
        } else if (f.type === 'textarea') {
            inputHtml = `<textarea name="${escapeHtml(f.key)}"${req}${ph} rows="3" class="lf-input"${autoFmtAttr}></textarea>`;
        } else {
            const inputType = f.type === 'phone' ? 'tel' : f.type === 'number' ? 'number' : f.type === 'email' ? 'email' : 'text';
            inputHtml = `<input type="${inputType}" name="${escapeHtml(f.key)}"${req}${ph} class="lf-input"${autoFmtAttr}>`;
        }
...
${step.fields.map((f) => {
        const swAttr = f.showWhen
            ? ` data-show-when-field="${escapeHtml(f.showWhen.field)}" data-show-when-equals="${escapeHtml(String(f.showWhen.equals))}"`
            : '';
        return `<div class="lf-field"${swAttr}><label class="lf-label">${escapeHtml(f.label)}${f.required ? '<span class="req">*</span>' : ''}</label>${renderField(f)}</div>`;
    }).join('\n')}
</div>`).join('\n')}
