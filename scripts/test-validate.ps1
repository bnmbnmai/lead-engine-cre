...
.DESCRIPTION
    Executes all backend (Jest) and Hardhat contract tests.
...
function Run-BackendTests {
    ...
    $suiteCount = if ($suiteMatch.Success) { $suiteMatch.Groups[1].Value } else { "?" }
    $failed = if ($failMatch.Success) { $failMatch.Groups[1].Value } else { "0" }
    
    if ($exitCode -eq 0) {
        Write-Pass "Backend: $passed tests passed - $suiteCount suites"
        $script:results += @{ Suite = "Backend Jest"; Status = "PASS"; Tests = $passed; Details = "$suiteCount suites" }
    } else {
        ...
        if ($output -match "FAIL") {
            ...
            $lines | Where-Object { $_ -match "FAIL" } | Select-Object -First 10 | ForEach-Object {
...
function Run-CypressTests {
    ...
    $cypressCmd = Join-Path "node_modules" ".bin" | Join-Path -ChildPath "cypress.cmd"
    if (-not (Test-Path $cypressCmd)) {
        Write-Skip "Cypress not installed - run: npm i cypress --save-dev"
    ...
    $output = npx cypress run --headless 2>&1 | Out-String
    $exitCode = $LASTEXITCODE
    
    $passMatch = [regex]::Match($output, '(\d+)\s+passing')
    $failMatch2 = [regex]::Match($output, '(\d+)\s+failing')
    $passed = if ($passMatch.Success) { $passMatch.Groups[1].Value } else { "?" }
    $failed = if ($failMatch2.Success) { $failMatch2.Groups[1].Value } else { "0" }
...
function Run-ArtilleryTests {
    ...
    if (-not $artilleryPath) {
        Write-Skip "Artillery not installed - run: npm i -g artillery"
        $script:results += @{ Suite = "Artillery"; Status = "SKIP"; Tests = "0"; Details = "not installed" }
        return
    }
    
    $configFile = Join-Path $ROOT "backend" | Join-Path -ChildPath "tests" | Join-Path -ChildPath "load-test.yml"
    if (-not (Test-Path $configFile)) {
        Write-Skip "No Artillery config found"
    ...
    $output = artillery run $configFile 2>&1 | Out-String
    $exitCode = $LASTEXITCODE
...
function Run-ContractTests {
    ...
    $passMatch = [regex]::Match($output, '(\d+)\s+passing')
    $failMatch3 = [regex]::Match($output, '(\d+)\s+failing')
    $passed = if ($passMatch.Success) { $passMatch.Groups[1].Value } else { "?" }
    $failed = if ($failMatch3.Success) { $failMatch3.Groups[1].Value } else { "0" }
...
Write-Host "  Started:   $dateStr" -ForegroundColor White
...
Write-Host "  Total: $passCount passed / $failCount failed / $skipCount skipped" -ForegroundColor $summaryColor
$durationStr = $elapsed.TotalSeconds.ToString('F1')
Write-Host "  Duration: ${durationStr}s" -ForegroundColor Gray
Write-Host ""
...
if ($Suite -eq "all") {
    ...
    Write-Host '    unit/ace             14 tests  jurisdiction + KYC + reputation' -ForegroundColor DarkGray
    ...
    Write-Host ""
    Write-Host '  Contracts Hardhat      PASS     ~15 tests    7 test files' -ForegroundColor DarkGreen
