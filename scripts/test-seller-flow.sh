...
# 3. Create Ask (roofing, FL)
# 4. Submit Lead (roofing, FL, API source)
# 5. Verify lead matched to ask
# 6. Check analytics
#
# Usage:
...
check_json_field() {
    ...
    local field="$2"
    local value
    value=$(echo "$json" | head -1 | python3 -c "import sys, json; d = json.load(sys.stdin); print(d$field)" 2>/dev/null || echo "MISSING")

    if [ "$value" != "MISSING" ] && [ "$value" != "None" ] && [ -n "$value" ]; then
        log_pass "Field $field = $value"
        echo "$value"
        return 0
    else
        log_fail "Field $field not found"
        return 1
    fi
}
...
# Note: Full auth requires wallet signing which can't be done in bash.
# For testing, we'll use the demo/integration endpoints instead.
echo "  (Full wallet auth requires signing — using demo endpoints for remaining tests)"

# ─── Step 3: Test Lead Submission Schema ─
...
# Should get 400 or 401 (bad data or no auth)
...
# ─── Step 7: Test Swagger Spec Exists ───
log_step "Swagger Spec Exists"
