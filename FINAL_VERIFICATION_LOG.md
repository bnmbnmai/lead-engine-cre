...
- **Real ERC-721 LeadNFTv2 Minting** — every auction winner gets a minted NFT on Base Sepolia with green "Minted #N ✓" badges and live Basescan token-page links (tokenIds 1–5 in March-3 run; #64 latest via auction-closure unification)
...
- **CRE DON 7-Gate Quality Scoring** — deterministic buyer-rule evaluation inside the Chainlink DON (vertical, geo, state, quality, off-site, verified, field-filters) with winner-only PII decrypt via CRE Confidential Compute. On-chain `requestOnChainQualityScore()` fires unconditionally after every NFT mint. The `demo-results` JSON waits for DON fulfillment (45s timeout) so every cycle contains a resolved on-chain score (0–100). The `cre-simulate` JSON remains the honest local pre-auction simulation.
...
- **Live Chainlink Automation** — Dedicated `PersonalEscrowVaultUpkeep` contract (upkeep ID 21294876610015716277122175951088366648605324800147651647408453016017624655922, 10 LINK funded, Active status) is registered to run 24h Proof-of-Reserves checks and auto-refunds expired bid locks. Verifiable on [automation.chain.link](https://automation.chain.link/base-sepolia/21294876610015716277122175951088366648605324800147651647408453016017624655922). Note: No upkeep actions have triggered yet on testnet (expected behavior — `checkUpkeep` only fires when conditions are met).
...
- **994/994 Tests** — 40 test suites, 9 deployed contracts, multi-track eligibility (CRE, Tokenization, DeFi, Privacy, Autonomous Agents)
