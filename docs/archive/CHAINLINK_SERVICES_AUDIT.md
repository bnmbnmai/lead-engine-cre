...
| **Data Feeds** (Price Guards) | `PersonalEscrowVault` (inline) | Reads Chainlink ETH/USD or USDC/USD feed in escrow deposit guard; reverts if price oracle stale | `backend/src/services/data-feeds.service.ts` | ✅ Live (price feed address in env) | [View ↗](https://sepolia.basescan.org/address/0x56bB31bE214C54ebeCA55cd86d86512b94310F8C) |
...
| **CRE Workflow** (Buyer Rules) | EvaluateBuyerRulesAndMatch | `CronCapability`, `ConfidentialHTTPClient`, `consensusIdenticalAggregation`, 7-gate rule evaluation | `cre-workflows/EvaluateBuyerRulesAndMatch/main.ts`, `backend/src/services/cre.service.ts` | ✅ Production workflow with `@chainlink/cre-sdk ^1.0.9` — auto-triggered on every lead in CRE-Native demo mode | N/A (DON-executed) |
| **CRE Workflow** (Winner Decryption) | DecryptForWinner | `ConfidentialHTTPClient`, `encryptOutput: true`, winner-only PII decryption | `cre-workflows/DecryptForWinner/main.ts`, `backend/src/routes/demo-panel.routes.ts` | ✅ Production workflow — PII decrypted only after `escrowReleased: true` verified | N/A (DON-executed) |
